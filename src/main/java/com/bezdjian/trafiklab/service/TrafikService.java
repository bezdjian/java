...
public class TrafikService {
  ...
  public TrafikService(JourneyPointRepository journeyPointRepository, TrafikLabAPIClient client) throws ClientException {
    this.journeyPointRepository = journeyPointRepository;
    this.client = client;

    log.info("***** Initializing TrafikService and saving stop points *****");
    saveAllPoints();
    // If we invoke findLineWithMostStops() so it will find and loop but it will not cache
